...
def mfed_selection(data_sub, X_m, fit_lengthscale=False,percentile=50):
    ...
    space = ParameterSpace([
                            ...
                            ContinuousParameter('Resolution', X[:,1].min(), X[:,1].max())])
    ...
    if fit_lengthscale:
        try:
            hmc = GPy.inference.mcmc.HMC(model_gpy)
            s = hmc.sample() # Burnin
...
def point_selection_algo(data_sub, resolution, budget, X_m, prev_val):
    ...
    match resolution:
        ...
        case "mfed" | "mfed_opt" | "mfed_75" | "mfed_50" | "mfed_90" | "mfed_early_75" | "mfed_early":
            closest_points = pd.DataFrame(sample_points_res(data_sub, prev_val +[2,8], X_m))
...
def calc_result(data):
    ...
    for budget in budget_list[:NUM_BUDGET]:
        ...
        while failure_count < 2:
            try:
                results = [i for i in calculate_optimal_density.map([data_tmp]*NUM_REP)]
